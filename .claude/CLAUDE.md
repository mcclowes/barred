...
make bump                # bump version (TYPE=patch|minor|major)
...
- `Utilities/` — helpers (AXExtensions)
- `Resources/` — Info.plist, Barred.entitlements, Assets.xcassets, AppIcon.icns

## Notes

- Requires Accessibility permission (System Settings > Privacy & Security > Accessibility)
- Tests in `Tests/BarredTests/` (run with `make test`)
- CI/CD: GitHub Actions workflows in `.github/workflows/` — `release.yml` (build, sign, notarize, GitHub release, Homebrew cask update), `ci.yml`, `bump-version.yml`, `tag-release.yml`
